...
def test_inference_smokers():
    ...
    mln = MLN(
        mlnfile=("%s:wts.pybpll.smoking-train-smoking.mln" % p),
...
def test_inference_taxonomies():
    ...
    mln = MLN(
        mlnfile=("%s:wts.learned.taxonomy.mln" % p),
        grammar="PRACGrammar",
        logic="FuzzyLogic",
    )
...
def test_learning_smokers():
    ...
    for method in ("BPLL", "BPLL_CG", "CLL"):
        for multicore in (True, False):
            ...
            learn(
                method=method, mln=mln, db=db, verbose=True, multicore=multicore
            ).run()
