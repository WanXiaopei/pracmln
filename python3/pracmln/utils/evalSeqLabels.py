...
def evalLabels(groundTruthFile, resultsFile, verbose=True):
    ...
    labels1 = [
        int(float(x.strip())) for x in file(groundTruthFile).readlines()
    ]  # ground truth
    labels2 = [
        int(float(x.strip())) for x in file(resultsFile).readlines()
    ]  # classification
    i = 1
    numCorrect = 0
    dist = 0
    while i < len(labels1) and i < len(labels2):
        ...
        if l1 != l2:
            ...
            #                    l  k i  j
            j = k = l = i
            while j < len(labels2) and labels2[j] == l2:
                j += 1  # search for next label in classification
            while k > 0 and labels1[k] == l1:
                k -= 1  # search for previous label in ground truth (-> must be equal to current label)
            while l > 0 and labels2[l] == l2:
                l -= 1  # search for segment start in classification
            ...
            #                   j i  k   l
            j = k = l = i
            while j > 0 and labels2[j] == l2:
                j -= 1  # previous label in classification (-> must match current label in ground truth)
            while k < len(labels1) and labels1[k] == l1:
                k += 1  # next label in ground truth (-> must match current label in classification)
            ...
            if verbose:
                print("%d: %d should have been %d [d=%.2f]" % (i, l2, l1, d))
        else:
            d = 0
            if verbose:
                print("%d: %d" % (i, l1))
    ...
    if verbose:
        ...
        print("\n\n*** Semantic Distance ***\n")
        print(
            "semantic distance: %f   implied accuracy: %f%%"
            % (ret["semanticErrors"], ret["semanticAccuracy"])
        )
