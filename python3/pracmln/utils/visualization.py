...
def get_cond_prob_png(queries, dbs, filename="cond_prob", filedir="/tmp"):
    ...
    if isinstance(dbs, str):
        ...
        for db in dbs:
            evidencelist.extend(
                [e for e in list(db.evidence.keys()) if db.evidence[e] == 1.0]
            )
    else:
        evidencelist.extend(
            [
                e if dbs.evidence[e] == 1.0 else "!" + e
                for e in list(dbs.evidence.keys())
            ]
        )
    ...
    return math2png(
        eq, filedir, declarations=DECLARATIONS, filename=safefilename, size=10
    )
