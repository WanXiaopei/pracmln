...
def to_html_hex(c1, c2, c3, model):
    ...
    (r, g, b) = (
        int(math.floor(r * 255.0)),
        int(math.floor(g * 255.0)),
        int(math.floor(b * 255.0)),
    )
    return "#%s%s%s" % (
        hex(r)[2:].ljust(2, "0"),
        hex(g)[2:].ljust(2, "0"),
...
class Node(object):
    ...
    def write(self, out):
        ...
        out.write(
            '<y:Geometry height="30.0" width="%s" x="%d" y="%d"/>'
            % (str(width), self.xpos, self.ypos)
        )
...
class Edge(object):
    ...
    def write(self, out):
        out.write(
            ...
            % (self.id, self.fromNode.id, self.toNode.id)
        )
        ...
        out.write(
            '<y:Arrows source="%s" target="%s"/>' % (self.sourceArrow, self.targetArrow)
        )
...
class UndirectedEdge(Edge):
    def __init__(self, graph, fromNode, toNode):
        Edge.__init__(
            self, graph, fromNode, toNode, sourceArrow="none", targetArrow="none"
        )
