...
class DirectDescent(object):
    ...
    def run(self):
        ...
        step = 1
        log.info(
            "starting optimization with %s... (alpha=%f)"
...
class DiagonalNewton(object):
    ...
    def run(self):
        ...
        while self.maxSteps is None or step <= self.maxSteps:
            ...
            if normg <= self.gtol:
                print(
                    "\nThreshold reached after %d steps. final gradient: %s (norm: %f <= %f)"
                    % (step, g.transpose(), normg, self.gtol)
                )
                break
            ...
            while True:
                ...
                if frac > 0.75:
                    l /= 2.0  # (lambda may become zero due to precision loss)
                elif frac < 0.25:
                    if l != 0.0:
                        l *= 4.0
                    else:
    ...
    def run(self):
        ...
        if optimizer == "bfgs":
            params = dict(
                [
                    k_v
                    for k_v in iter(self.optParams.items())
                    if k_v[0] in ["gtol", "epsilon", "maxiter"]
                ]
            )
            if self.verbose:
                print("starting optimization with %s... %s\n" % (optimizer, params))
            wt, f_opt, grad_opt, Hopt, func_calls, grad_calls, warn_flags = fmin_bfgs(
                neg_f, self.wt, fprime=neg_grad, full_output=True, **params
            )
            if self.verbose:
            ...
            params = dict(
                [
                    k_v1
                    for k_v1 in iter(self.optParams.items())
                    if k_v1[0] in ["gtol", "epsilon", "maxiter"]
                ]
            )
            log.info("starting optimization with %s... %s" % (optimizer, params))
            wt, f_opt, func_calls, grad_calls, warn_flags = fmin_cg(
                neg_f, self.wt, fprime=neg_grad, args=(), full_output=True, **params
            )
            ...
            params = dict(
                [
                    k_v2
                    for k_v2 in iter(self.optParams.items())
                    if k_v2[0] in ["avextol", "epsilon", "maxiter"]
                ]
            )
            log.info("starting optimization with %s... %s" % (optimizer, params))
            wt, f_opt, func_calls, grad_calls, warn_flags = fmin_ncg(
                neg_f, self.wt, fprime=neg_grad, args=(), full_output=True, **params
            )
            log.info("optimization done with %s..." % optimizer)
            log.info(
            ...
            params = dict(
                [
                    k_v3
                    for k_v3 in iter(self.optParams.items())
                    if k_v3[0] in ["xtol", "ftol", "maxiter"]
                ]
            )
            log.info("starting optimization with %s... %s" % (optimizer, params))
            ...
            params = dict(
                [
                    k_v4
                    for k_v4 in iter(self.optParams.items())
                    if k_v4[0] in ["xtol", "ftol", "maxiter"]
                ]
            )
            log.info("starting optimization with %s... %s" % (optimizer, params))
            ...
            params = dict(
                [
                    k_v5
                    for k_v5 in iter(self.optParams.items())
                    if k_v5[0] in ["gtol", "epsilon", "maxiter", "bounds"]
                ]
            )
            log.info("starting optimization with %s... %s" % (optimizer, params))
            if "bounds" in params:
